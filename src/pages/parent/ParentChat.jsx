...
export default function ParentChat() {
  ...
  async function init() {
    ...
    // Find teacher of child's class
    const { data: kids } = await supabase
      .from('children').select('class_id').eq('parent_id', profile.id).limit(1)
    const classId = kids?.[0]?.class_id
    if (!classId) { setNotFound(true); setLoading(false); return }

    const { data: cls } = await supabase
      .from('classes').select('teacher_id').eq('id', classId).maybeSingle()
    const staffId = cls?.teacher_id
    if (!staffId) { setNotFound(true); setLoading(false); return }

    // Get staff name
    const { data: sp } = await supabase
      .from('profiles').select('name, email, role').eq('id', staffId).maybeSingle()
    setStaffName(sp?.name || sp?.email?.split('@')[0] || 'Lehrer')
    setStaffRole(sp?.role || 'teacher')
    ...
    if (!conv) {
      const { data: newConv } = await supabase.from('conversations')
        .insert({ instructor_id: staffId, user_id: profile.id, user_role: 'parent' })
  ...
  if (notFound || !convId) return (
    <div className="page-content">
      <div style={{ padding: 40, textAlign: 'center', opacity: .45, lineHeight: 1.7 }}>
        ...
        <div style={{ fontWeight: 600 }}>Kein Lehrer gefunden</div>
        <div style={{ fontSize: 13 }}>Bitte kontaktiere deine Schwimmschule.</div>
      </div>
    </div>
  )
