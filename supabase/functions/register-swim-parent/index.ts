...
serve(async (req) => {
  ...
  try {
    ...
    const { email, password, firstName, lastName, birthDate, swimCourseId, avatar } =
      payload as Record<string, string>

    const missing = ['email', 'password', 'firstName', 'lastName', 'swimCourseId'].filter(k => !payload[k])
    if (missing.length > 0) {
    ...
    // ── 4. Insert child linked to swim course ─────────────────
    const { data: child, error: childError } = await supabase
      ...
      .insert({
        ...
        level:              'seepferdchen',
        avatar:             avatar || '👦',
        swim_course_id:     swimCourseId,
        swim_payment_status: 'pending',
      })
      .select('id')
      .single()
