...
serve(async (req) => {
  ...
  try {
    ...
    const rows: unknown[][] = [
      ...
      ['Formulaire B2 — Confirmation Examen'],
      [],
      [course.name],
      [],
      ...
      ['Date examen:', examDate],
      [],
      ['N°', 'Nom', 'Prénom', 'Date de naissance', 'Adresse Mail', 'Junior Lifesaver', 'Lifesaver'],
    ]
    ...
    for (let i = 0; i < participants.length; i++) {
      ...
      rows.push([
        ...
        p.email || '—',
        p.status === 'passed_junior' ? '✓' : '',
        p.status === 'passed_lifesaver' ? '✓' : '',
      ])
    }

    const ws = XLSX.utils.aoa_to_sheet(rows)
    ws['!cols'] = [{ wch: 5 }, { wch: 20 }, { wch: 20 }, { wch: 18 }, { wch: 30 }, { wch: 16 }, { wch: 12 }]
    ...
    const participantRows = participants.map((p, i) => `
      <tr style="border-bottom:1px solid #ddd;">
        ...
        <td style="padding:8px;text-align:center;">${p.status === 'passed_junior' ? '✓' : ''}</td>
        <td style="padding:8px;text-align:center;">${p.status === 'passed_lifesaver' ? '✓' : ''}</td>
      </tr>
    `).join('')

    const html = `
      <div style="font-family:Arial,sans-serif;max-width:750px;margin:0 auto;color:#111;padding:24px;">
        <div style="background:#1a5c2a;color:white;padding:20px 24px;border-radius:8px 8px 0 0;">
          ...
          <div style="font-size:22px;font-weight:700;">Formulaire B2 — Confirmation Examen</div>
          <div style="font-size:13px;opacity:.8;margin-top:4px;">${course.name}</div>
        </div>
        ...
        <div style="background:#f5f5f5;border:1px solid #ddd;border-top:none;padding:20px 24px;margin-bottom:20px;border-radius:0 0 8px 8px;">
          <table style="width:100%;border-collapse:collapse;">
            ...
            <tr>
              <td style="padding:6px 0;color:#666;font-size:13px;">Date examen:</td>
              <td style="padding:6px 0;font-weight:700;color:#1a5c2a;font-size:15px;">${examDate}</td>
              <td></td><td></td>
            </tr>
          </table>
        </div>
        ...
        <div style="font-weight:700;font-size:15px;margin-bottom:12px;">
          Candidats admis (${participants.length})
        </div>
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <thead>
            <tr style="background:#1a5c2a;color:white;">
              ...
              <th style="padding:10px;text-align:center;">Junior Lifesaver</th>
              <th style="padding:10px;text-align:center;">Lifesaver</th>
            </tr>
          </thead>
        ...
        <div style="margin-top:24px;padding:16px;background:#e8f0fe;border:1px solid #c5d8f7;border-radius:8px;font-size:13px;color:#1a3a6e;">
          <strong>📎 Anhang:</strong> Das ausgefüllte Formulaire B2 ist als Excel-Datei beigefügt.
        </div>

        <div style="margin-top:16px;padding:16px;background:#d4edda;border:1px solid #c3e6cb;border-radius:8px;font-size:13px;color:#155724;">
          <strong>✓ Bestätigung:</strong> Die Zahlungen der Abzeichen-Gebühren werden durch Letzschwamm direkt an die FLNS weitergeleitet. Kein separater Zahlungsnachweis erforderlich.
        </div>
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `FLNS Formulaire B2 — ${course.name} — Examen ${examDate}`,
        html,
    ...
    await supabase.from('sauvetage_courses').update({
      b2_sent_at: new Date().toISOString(),
      status: 'exam_done',
    }).eq('id', courseId)
